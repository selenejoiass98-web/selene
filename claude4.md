...
### Rastreio real + correções do checkout (sessão 11/07/2026)
- [x] **`api/rastreio.js`**: busca pedidos no Firestore via REST com **service account** (env `FIREBASE_SERVICE_ACCOUNT` na Vercel — JSON completo da chave). A coleção `pedidos` NÃO tem leitura pública (contém CPF/endereço/telefone — confirmado por teste: `permission-denied`) e deve continuar assim. A API devolve só campos seguros (numeroPedido, status, mercadoPago.status, itens nome/variante/preço/qty, total, frete nome/desc, codigoRastreio, transportadora, primeiroNome). Token OAuth2 via JWT RS256 assinado com `node:crypto` (sem SDK), cache em módulo
- [x] **`rastreio.html` conectado a pedidos reais** (DEMO_ORDERS removido): busca por número (#EC-xxxxx), código dos Correios (campo `codigoRastreio`) ou **e-mail OU CPF** (aba renomeada; CPF aceito com/sem máscara; servidor tenta mascarado e cru). Status inclui "Aguardando pagamento" (novo + mercadoPago.status ≠ approved) e "Cancelado"; timeline derivada do status; vários pedidos do mesmo e-mail/CPF viram lista de pills clicáveis. Link WhatsApp em negrito/sublinhado
- [x] **Checkout — modal de sucesso por método**: boleto mostra "Pedido registrado!" + "aguardando o pagamento do boleto" (não "obrigada pela sua compra"); demais métodos mantêm o texto original. ids `success-title`/`success-sub`
- [x] **`enviarEmailConfirmacao()` implementada** — era CHAMADA no `mostrarSucesso()` mas nunca existiu: cartão quebrava com ReferenceError e o carrinho nem era limpo. Template de marca (mesmo padrão do boleto), com link direto pro rastreio (`rastreio.html?pedido=...`). Pix também envia (chamada no `processarPix`). Envios agora checam `res.ok` e logam falha no console
- [x] **Diagnóstico e-mail de boleto "não chegou"** (10/07): `/api/email` testado em produção → 200 + messageId, canal OK. Provável spam do Hotmail (o BCC em contato@ permite confirmar se o envio original saiu). Falha silenciosa era possível porque a resposta era ignorada — corrigido
- [x] env `FIREBASE_SERVICE_ACCOUNT` criada na Vercel (11/07) — /api/rastreio testado em produção com pedido real: busca, badge "Aguardando pagamento", timeline e itens OK
- [x] **BUG CRÍTICO corrigido (11/07): NENHUM pedido era salvo no Firestore desde sempre** — `salvarPedidoFirebase()` importava `firebase-firestore` **10.12.0** dinamicamente enquanto a página usa **10.7.1**; o `addDoc` da 10.12.0 rejeita a CollectionReference da 10.7.1 e caía no `catch` silencioso. A coleção `pedidos` estava VAZIA (confirmado via service account). Junto morriam a baixa de estoque (`atualizarEstoque`) e o cadastro no Brevo (vinham depois no mesmo try). Fix: `window._fbAddDoc` exportado do módulo 10.7.1. **REGRA: nunca importar um segundo firebase-firestore de versão diferente na mesma página**
- [ ] Limpar 2 pedidos de teste da coleção `pedidos` (Firebase Console → Firestore): doc `HU7WXnzg8p9wTcn8u6Me` (diagnóstico) e o `#EC-90001` (teste E2E do rastreio)
- Regras do Firestore (verificado 11/07 por teste): `pedidos` permite `create` público, mas `read`/`delete` públicos são NEGADOS (correto — dados pessoais; o rastreio passa pela API)

### Brevo Contatos + Canal WhatsApp (sessão 10/07/2026)
- [x] **`api/contact.js`** (nova serverless function): cadastra/atualiza contato na lista **"Clientes do Site"** do Brevo via `POST /v3/contacts` (`updateEnabled:true` — comprar de novo não duplica). A lista (e a pasta, se não existir) é **criada automaticamente na primeira chamada** (lookup por nome, cache do ID em módulo). Telefone vai no atributo `SMS` normalizado para `+55...`; se o Brevo rejeitar o telefone, refaz sem ele (o e-mail nunca deixa de ser cadastrado por causa do telefone)
- [x] **Checkout → Brevo**: `salvarPedidoFirebase()` em `checkout.html` chama `/api/contact` fire-and-forget (`.catch(()=>{})`) logo após salvar o pedido — erro no Brevo jamais atrapalha a venda. Todo comprador (Pix/Cartão/Boleto) entra na lista
- [x] **Seção Newsletter da home substituída por CTA do canal do WhatsApp** (`https://whatsapp.com/channel/0029Vb7udKDCxoB1SH7Ry22U`): botão verde WhatsApp (`.newsletter-wa`) "Entrar na lista de transmissão" + textos novos. A função fake `subscribeNewsletter()` (só mostrava toast, não salvava nada) foi removida, junto com o CSS do formulário
- [x] **Atenção**: os ids `news-tag/news-title1/news-title2/news-sub/news-btn/news-privacy` foram mantidos — os textos continuam editáveis pelo admin (card Newsletter em `configuracoes.html`) e sobrescrevem os novos defaults se houver valores salvos no Firestore (`configuracoes/loja → newsletter`). O `news-btn` agora é um `<span>` dentro do link (pra não apagar o ícone SVG ao aplicar texto do admin)
- [x] Contexto: e-mails transacionais (`smtp/email`) NÃO criam contatos no Brevo — por isso compradores não apareciam em Contatos/Listas. Instagram da home: embeds morrem se a conta virar privada (aconteceu em 10/07 — post não renderiza e fica espaço em branco; solução: voltar a público ou desligar o toggle no admin)
